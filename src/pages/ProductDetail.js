...
const ProductDetail = () => {
  ...
  useEffect(() => {
    axios.get(`http://16.171.166.44:5000/api/products/${id}`)
      .then(res => {
        setProduct(res.data);
        setLoading(false);
      })
  ...
  const handleAddToCart = async () => {
    ...
    try {
      const userRes = await axios.get('http://16.171.166.44:5000/api/auth/profile', {
        headers: { Authorization: `Bearer ${token}` },
      });
      const user_id = userRes.data.id;

      await axios.post(
        'http://16.171.166.44:5000/api/cart/add',
  ...
  const handleWishlist = async () => {
    ...
    try {
      const profileRes = await axios.get('http://16.171.166.44:5000/api/auth/profile', {
        headers: { Authorization: `Bearer ${token}` },
      });
      const userId = profileRes.data.id;

      await axios.post(
        'http://16.171.166.44:5000/api/wishlist/add',
