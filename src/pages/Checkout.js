...
const Checkout = () => {
  ...
  useEffect(() => {
    ...
    axios.get('http://16.171.166.44:5000/api/auth/profile', {
      ...
      .then(res => {
        ...
        return axios.get(`http://16.171.166.44:5000/api/address/${userData.id}`);
      })
      .then(addrRes => {
  ...
  useEffect(() => {
    if (productId) {
      axios.get(`http://16.171.166.44:5000/api/products/${productId}`)
        .then(res => setSingleProduct(res.data))
        .catch(() => alert('Product not found'));
    } else if (stateCart) {
      setCartItems(stateCart);
    } else if (user?.id) {
      axios.get(`http://16.171.166.44:5000/api/cart/${user.id}`)
